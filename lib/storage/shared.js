const { checkStatus, urlEncodeData } = require('../utils')
const {
  EAKInfo,
  SignedDocument,
  Note,
...
const GroupData = require('../../types/groupData')
const GroupKeys = require('../../types/groupKeys')
...
async function createGroupRequest(
  ...
  const eak = await crypto.encryptPrivateKey(
    groupKeyPair.privateKey,
    encryptionKeyPair
  )
  let groupData = new GroupData(name, capabilities)
  let groupKeys = new GroupKeys(encryptionKeyPair.publicKey, eak)
  let group = new Group(groupData, groupKeys)
  return group
}

/*
 * createGroup is an internal method used to create a group
 */
async function createGroup(
  ...
  let createGroupReq = await createGroupRequest(
    crypto,
    name,
    capabilities,
    encryptionKeyPair
  )
  let response = await authenticator.tsv1Fetch(`${apiUrl}/v2/storage/groups`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
    body: createGroupReq.stringify(),
  })
  let storedGroupResp = await checkStatus(response)
  let groupJson = await storedGroupResp.json()
  let group = Group.decode(groupJson)
  return group
}

async function readGroup(authenticator, groupID, apiUrl) {
  let request = await authenticator.tsv1Fetch(
    `${apiUrl}/v2/storage/groups/${groupID}`,
    {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
  ...
  let storedGroup = await checkStatus(request)
  let groupJson = await storedGroup.json()
  let group = Group.decode(groupJson)
  return group
}
...
async function listGroups(
  ...
  // let requestUrl = `${apiUrl}/v2/storage/groups`
  // do some checking & add in required elements
  // utils/urlEncodeData from index.js for this
  const urlData = {
    client_id: clientID,
  ...
  // let fullReqUrl = requestUrl + '?' + encodedUrl
  // console.log(fullReqUrl)
  let request = await authenticator.tsv1Fetch(
    `${apiUrl}/v2/storage/groups?${encodedUrl}`,
    // `${apiUrl}/v2/storage/groups`,
    {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
  ...
  console.log(request)
  let storedGroups = await checkStatus(request)
  let groupsJson = await storedGroups.json()
  console.log(groupsJson)
  return groupsJson
  // let listGroups = ListGroups.decode(groupsJson)
  // return listGroups
}
