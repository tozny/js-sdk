...
class Storage {
  ...
  async writeNote(
    ...
    var decodedOptions = NoteOptions.decode({
      ...
      record_id: options.record_id?options.record_id : null,
  ...
  /**
   * getNoteViewsLeft is used to retrieve the number of views/decrypts left for 
   * an anonymous note.
   *
   * @param {string} noteId  UUID assigned by TozStore, used to identify a note.
   * @param {KeyPair} signingKeyPair Object that has signing public and private keys
   ...
   */
    async getNoteViewsLeft(
      ...
      const params = Object.assign({}, authParams, { note_id: noteId })
      return shared.getNoteViewsLeft(
        anonAuth,
        params,
        authHeaders,
        apiUrl
  ...
  async writeNoteFile(
    ...
    let { size, checksum, tempFile } = await this.crypto.encryptFile(file, accessKey, this.fileOps)
    ...
    let encryptedNote = await shared.createEncryptedNoteWithAK(
      this.crypto,
      data,
      recipientEncryptionKey,
      recipientSigningKey,
  ...
  async getNoteFile(
    ...
    const params = Object.assign({}, authParams, { note_id: noteId })
    let note = await shared.readNote(
      this.crypto,
      anonAuth,
      encryptionKeyPair,
      params,
    ...
    const ak = await this.crypto.decryptNoteEak(encryptionKeyPair.privateKey, { eak: note.encryptedAccessKey }, note.writerEncryptionKey)
    return new NoteFile(this.crypto, ak, note, this.fileOps)
  }
