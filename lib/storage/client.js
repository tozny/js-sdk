...
class Client extends CryptoConsumer {
  ...
  async listRecordsSharedWithGroup(
    ...
    for (var i = 0; i < recordsJson.results.length; i++) {
      ...
      let decryptedRecord = await shared.decryptRecord(this, record)
  ...
  async shareRecordWithGroup(groupId, recordType) {
    ...
    let groupInfo = this.readGroup(groupId)
    // Get the encrypted Access Key
    let encryptedAccessKey = await this.getReaderKey(
      this.config.clientId,
      this.config.clientId,
      recordType
    )
    ...
    const shareRecordRequest = {
      ...
      encrypted_access_key: encryptedAccessKey.eak,
