...
class Client extends CryptoConsumer {
  ...
  async search(searchRequest) {
    ...
    console.log(`in search, calling SearchResult with ${JSON.stringify(searchRequest)}`)
    return new SearchResult(this, searchRequest)
  }

  async _search(searchRequest) {
    console.log(` in _search calling endpoint with ${JSON.stringify(searchRequest)}`)
    ...
    await checkStatus(response)
    console.log(`response in _search ${JSON.stringify(response)}`)
    return response.json()
  }
   ...
   */
   async listRecordsSharedWithGroup(
    ...
    console.log(`from client nextToken = ${nextToken} max = ${max}`)
    ...
    console.log(`results = ${JSON.stringify(resultList)}`)
