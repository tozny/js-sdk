...
class Client extends CryptoConsumer {
  ...
  async revokeRecordForMe(writerId, type, readerId) {
    ...
    let result = await shared.deleteAccessKeyByReader(this, writerId, writerId, readerId, type)
    if(result){
      let policy = { deny: [{ read: {} }] }
      return shared.putPolicyByReader(this, policy, writerId, writerId, readerId, type)
    } 
    return result;
  ...
  async putUserProfilePic(realmName, file) {
    const urlReq = await this.authenticator.tsv1Fetch(
      `${this.config.apiUrl}/v2/storage/realms/${realmName}/profile_picture`,
      {
        method: 'POST',
        body: JSON.stringify({
  ...
  async getUserProfilePic(realmName, userId) {
    const response = await this.authenticator.tsv1Fetch(
      `${this.config.apiUrl}/v2/storage/realms/${realmName}/profile_picture/${userId}`,
      {
        method: 'GET'
      }
    )
  ...
  async isRecordLockedForEditing(recordId) {
    const response = await this.authenticator.tokenFetch(
      `${this.config.apiUrl}/v1/storage/records/${recordId}/lock_status`,
      {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
			...
			// Assuming the response is JSON and contains a boolean field indicating lock status
		const data = await response.json();
    return data;
  }


  async lockRecordForEditing(recordId,version, typeOfMethod,isLocked) {
    const response = await this.authenticator.tokenFetch(
      `${this.config.apiUrl}/v1/storage/records/${recordId}/${version}/lock`,
      {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
