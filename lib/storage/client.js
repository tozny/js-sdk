...
class Client extends CryptoConsumer {
  ...
  /**
   * Transforms a file Record or the ID of a file Record into a File object 
   * and decrypts the record data from file record.
   *
   * @param {Record|string} file Either a file Record or a file record ID.
   */
  async getFileWithDecryptedRecordData(file) {
    let fileObj = this.getFile(file)
    const fileRecord = await fileObj.record()
    const decryptedFileRecord = await shared.decryptRecord(this, fileRecord)
    fileObj._record = decryptedFileRecord
    return fileObj
  }
