...
class Client extends CryptoConsumer {
  ...
  /**
   * createGroup is a static method that creates a groups and sends it to TozStore,
   * allowing you to supply your own encryptionKey pair.
   *
   *
   * @param {object} data  A hashmap of the data in the group - for now this just includes groupName
   * @param {string} recipientEncryptionKey The public encryption key of the reader of this note
   *
   * @returns {Group} A response from TozStore; the group that has been written.
   */
  async createGroup(data, recipientEncryptionKey) {
    /* eslint-enable */
    const encryptionKeys = new KeyPair(
      this.config.publicKey,
      this.config.privateKey
    )
    return shared.createGroup(
      this.crypto,
      this.authenticator,
      data,
      encryptionKeys,
  ...
  async readGroup(groupId) {
    return shared.readGroup(this.authenticator, groupId, this.config.apiUrl)
  }
}
