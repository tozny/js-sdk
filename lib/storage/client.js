...
class Client extends CryptoConsumer {
  ...
  async createGroup(name, capabilities = [], description) {
    ...
    for (const key of createGroupReqKeys) {
      if (createGroupReq[key] === null || (Array.isArray(createGroupReq[key]) && createGroupReq[key].length === 0)) {
...
/**
   *
   * @param {string} realm_name  User's  realm Name
   *
   * @return {Object}  All groups associted with that Realm
   */
  async listAllGroupsForAdmin(realm_name){

      let response = await this.authenticator.tsv1Fetch(
        `${this.config.apiUrl}/v2/storage/groups/realm/${realm_name}`,
        {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
  ...
  async listGroupsByID(groupIDs) {
    ...
    let response = await this.authenticator.tsv1Fetch(
      `${this.config.apiUrl}/v2/storage/groups/bulk?${encodedUrl}`,
      {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
