...
class Client extends CryptoConsumer {
  ...
  async putUserProfilePic(realmName, file,picType) {
  ...
  async getUserProfilePic(realmName, picType) {
    const response = await this.authenticator.tsv1Fetch(
      `${this.config.apiUrl}/v2/storage/realms/${realmName}/profile_picture/${picType}`,
      {
        method: 'GET'
      }
    )
