...
class Client extends CryptoConsumer {
  ...
  async bulkListGroupMembers(groupIds) {
    ...
    let groupMembersJson = await validateResponseAsJSON(response)
    return new Map(Object.entries(groupMembersJson.results));
  }
  /**
   ...
   *
   * @return {Array}  An object containing the nextToken and a map of group IDs to records shared with that group.
   */
  async bulkListRecordsSharedWithGroup(groupIds, nextToken = '', max = 100) {
    ...
    let endpoint = `${this.config.apiUrl}/v2/storage/groups/bulk/share?${encodedUrl}`
    ...
    for (const [groupID, records] of Object.entries(recordsJson.results)) {
      ...
      groupRecords.set(groupID, listSharedWithGroups)
    }
