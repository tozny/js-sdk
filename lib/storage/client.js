...
class Client extends CryptoConsumer {
  ...
  /**
   * Transforms a file record or file record ID into a file object.
   *
  ...
  async webAuthn(name, displayName) {
    const challengeArgs = {
      name,
      display_name: displayName,
    }
    const challengeRequest = await this.authenticator.tsv1Fetch(
      `${this.config.apiUrl}/v2/storage/notes/webauthn`,
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify(challengeArgs),
      }
    )
    const { challenge, challengeId } = await validateResponseAsJSON(
      challengeRequest
    )
    challenge.publicKey.user.id = this.crypto.platform.b64decode(
      challenge.publicKey.user.id
    ).buffer
    challenge.publicKey.challenge = this.crypto.platform.b64decode(
      challenge.publicKey.challenge
    ).buffer
    // eslint-disable-next-line no-undef
    const signed = await navigator.credentials.create(challenge)
    const rawId = this.crypto.platform.b64URLEncode(
      new Uint8Array(signed.rawId)
    )
    const attestationObject = this.crypto.platform.b64URLEncode(
      new Uint8Array(signed.response.attestationObject)
    )
    const clientDataJSON = this.crypto.platform.b64URLEncode(
      new Uint8Array(signed.response.clientDataJSON)
    )
    return {
      challengeId,
      challengeResponse: {
