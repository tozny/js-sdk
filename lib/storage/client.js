...
class Client extends CryptoConsumer {
   ...
   * @param {string} params.nextToken - The token for the next page of results.
  ...
  async fetchGroupIDsByCapabilities(params) {
    const url = new URL(`${this.config.apiUrl}/v2/storage/groups/client/${params.clientId}`);
    ...
    const response = await this.authenticator.tsv1Fetch(url, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
      },
