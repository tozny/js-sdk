 ...
 */
  ...
  async createGroup(name, capabilities = [], description) {
    ...
    const groupMembers = await this.listGroupMembers(group.groupID)

    if (groupMembers.length !== 1 || groupMembers[0].client_id !== this.config.clientId) {
      throw new Error("Error creating new group. ");
    }
    const groupMembership = new GroupMembership(this.config.clientId, group, groupMembers[0].capability_names);
  ...
  async addRealmGroupMember(payload) {

    const tozStoreGroupInfo = await this.readGroup(payload.tozStoreId);

    const realmName = payload.realmName;
    const memberId = payload.newMember;
    const groupId = JSON.parse(tozStoreGroupInfo.description).tozId
    const group = {
      groups: [groupId],
    }
  ...
  async listAllRealmRoles(realmName){
    ...
    const roles  = await validateResponseAsJSON(response)
    return roles;
  }
