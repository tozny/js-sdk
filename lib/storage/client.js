...
class Client extends CryptoConsumer {
  ...
  /**
     ...
     */
  async listAllGroupsForAdmin(realmName, nextToken = null,
    max = null) {
    const urlData = {
      nextToken: nextToken,
      max: max,
  ...
  async bulkListGroupMembers(groupIds, nextToken = null, max = 100) {
    ...
    console.log(listGroupMembersRequest)
    ...
    let encoded = `${encodedUrl}&${encodedUrlMax}`
    console.log(encoded)
    // Call the API endpoint to remove group members
    let response = await this.authenticator.tsv1Fetch(
      `${this.config.apiUrl}/v2/storage/groups/bulk/members?${encoded}`,
      {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
