...
class Client extends CryptoConsumer {
  ...
  /**
   * WriteSecretNote is a static method that encrypts a note and sends it to TozStore,
  ...
  async writeSecretNote(
    ...
    let authorizer =  new AuthenticatedRequest(config, this.crypto)
   ...
   * @param {string} noteId  UUID assigned by TozStore, used to identify a note.
   */
    async deleteAllRecordNotes(recordId) {
      if (this.config.version === 1) {
        throw new Error('Cannot delete notes without a signing key!')
      }
      // Use this.constructor to ensure we referencing the implementing class.
      return shared.deleteAllRecordNote(this.authenticator, recordId, this.config.apiUrl)
    }
   ...
   * @param {string} noteId  UUID assigned by TozStore, used to identify a note.
   */
  async getAllRecordNotes(recordId,nextToken = null, max = null) {
    if (this.config.version === 1) {
      throw new Error('Cannot delete notes without a signing key!')
    }
    // Use this.constructor to ensure we referencing the implementing class.
    return shared.getAllRecordNotes(this.authenticator, recordId, this.config.apiUrl,nextToken,max)
  }
