...
class Crypto {
  ...
  async decryptEak(readerKey, encryptedAk) {
    ...
    if (
      ...
      for (let akWrapper of encryptedAk.access_key_wrappers) {
        privateKey = await this.decryptEakLayer(
          privateKey,
          this.platform.b64URLDecode(akWrapper.public_key),
    ...
    return this.decryptEakLayer(
      privateKey,
      this.platform.b64URLDecode(rawPublicKey.curve25519),
      encryptedAk.eak
    )
  }
  /**
  ...
  async decryptRecord(encrypted, accessKey) {
    ...
    let meta
    if (encrypted.meta instanceof types.Meta) {
      meta = types.Meta.decode(encrypted.meta.serialize())
    } else {
      meta = types.Meta.decode(encrypted.meta)
    }
