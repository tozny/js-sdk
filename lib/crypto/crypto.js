...
const { DEFAULT_KDF_ITERATIONS } = require('../utils/constants')
...
class Crypto {
  ...
  async decryptString(encrypted, key) {
    ...
    const decrypted = this.provider.decryptSymmetric(cipher, nonce, rawKey)
  ...
  async signField(key, value, signingKey, objectSalt) {
    ...
    const message = await this.provider.hash(`${salt}${key}${value}`)
  ...
  async verifyField(key, value, verifyingKey, objectSalt) {
    ...
    const message = await this.provider.hash(`${parts[1]}${key}${plainText}`)
  ...
  async decryptRecord(encrypted, accessKey) {
    ...
    for (let key in encrypted.data) {
      ...
      if (encrypted.data.hasOwnProperty(key)) {
        decrypted.data[key] = await this.decryptField(
          encrypted.data[key],
          accessKey
        )
      }
  ...
  async deriveSigningKey(seed, salt, iterations = DEFAULT_KDF_ITERATIONS) {
    const keypair = await this.provider.deriveSigningKey(seed, salt, iterations)
  ...
  async deriveCryptoKey(seed, salt, iterations = DEFAULT_KDF_ITERATIONS) {
    const keypair = await this.provider.deriveCryptoKey(seed, salt, iterations)
  ...
  async deriveSymmetricKey(
    ...
    const buffer = await this.provider.deriveSymmetricKey(
      password,
      salt,
      iterations
    )
