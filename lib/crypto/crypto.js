...
class Crypto {
  ...
  async verifyField(key, value, verifyingKey, objectSalt) {
    ...
    // Every field must carry a signature. Anything else is rejected.
    if (parts[0] !== this.signatureVersion) {
        throw new Error(`Missing field signature for "${key}"`)
    }
    ...
    if (!/^\d+$/.test(parts[2])) throw new Error(`Invalid field signature header for "${key}"`)
    const headerLength = parts.reduce((acc, part) => acc + part.length, 3)
    const signatureLength = parseInt(parts[2], 10)
    // A non-numeric length would make the signature and plain text offsets NaN,
    // which `substring` silently coerces rather than failing.
    if (!Number.isInteger(signatureLength) || signatureLength < 0) {
      throw new Error(`Invalid field signature header for "${key}"`)
    }
  ...
  async decryptRecord(encrypted, accessKey) {
    ...
    let decrypted = new types.Record(
      meta,
      data,
      encrypted.signature,
      encrypted.clientsSharedWith
    )
    return decrypted
  }
  ...
  async decryptNote(encrypted, accessKey, verifyingKey) {
    ...
    if (typeof sig !== 'string' || !sig.startsWith(this.signatureVersion + ';')) {
      throw new Error('Unable to verify note: missing or unsupported signature')
    }
    // Verified salt comes from the note's own signed "signature" field
    const signatureSalt = await this.verifyField('signature', sig, verifyingKey)
    if (!signatureSalt) {
      throw new Error('Unable to verify note: empty signature salt')
    }
    const decrypted = types.Note.clone(encrypted)
    const decryptedData = {}
    for (const key of Object.keys(encrypted.data)) {
  ...
  async decryptFile(source, accessKey, ops) {
    ...
    // destination stream immediately. Because it isn't awaited, any failure --
    // a wrong key, corrupt cipher text, or a truncated file -- has to be pushed
    // onto the destination stream. Otherwise it surfaces as an unhandled
    // rejection and the consumer waits on a stream that never closes.
    transformer.transform(source).catch((e) => {
      if (typeof destination.error === 'function') {
        destination.error(e)
      } else if (typeof destination.abort === 'function') {
        destination.abort(e)
      } else {
        // Can't signal the failure to the reader, so at least don't hide it
