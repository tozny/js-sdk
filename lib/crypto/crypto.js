...
class Crypto {
  ...
  /**
   * Encrypt a private key using a keypair.
   *
   ...
   */
  async encryptPrivateKey(privateKey, encryptionKeyPair) {
    let rawPrivateKey = this.platform.b64URLDecode(privateKey)
    let publicEncKey = this.platform.b64URLDecode(encryptionKeyPair.publicKey)
    let privateEncKey = this.platform.b64URLDecode(encryptionKeyPair.privateKey)
