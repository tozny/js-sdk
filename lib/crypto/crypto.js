...
class Crypto {
  ...
  async encryptFile(handle, accessKey, ops) {
    ...
    const tempFile = ops.tempFileForWrite()
    ...
    // Encrypt the file
    const reader = ops.readStream(handle, FILE_BLOCK_SIZE)
    do {
      const block = stream.encrypt(await reader.next(), reader.done)
      md5.update(block)
  ...
  async decryptFile(fileUrl, accessKey, ops) {
    const source = await this._fileOps.download(fileUrl)
