...
async function checkStatus(response) {
  ...
  const resolvedStatusText = response.statusText || statusTexts[response.status] || 'Unknown Error';

  throw new errors.general.RequestError(response, resolvedStatusText)
}
...
function urlEncodeData(element, key, processed = []) {
  if (typeof element === 'object') {
    ...
    processed.push(`${key}=${encodeURIComponent(element).replace(/%20/g, "+")}`)
  }
...
async function credentialedDecodeResponse(response) {
  try {
    ...
    switch (e.response.status) {
      ...
      case 400:
        throw new errors.identity.TokenExpiredError(e.response, e.message)
      default:
        throw e
    }
  }
