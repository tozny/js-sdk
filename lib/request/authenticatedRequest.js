...
class AuthenticatedRequest {
  ...
  async tokenAuth() {
    ...
    const expiry = Date.parse(json.expires_at)
    // An unparsable expiry must not read as "never expires" -- treat the token
    // as already stale so the next request fetches a fresh one.
    this._authTokenTimeout = isNaN(expiry) ? 0 : expiry
