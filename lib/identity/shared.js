...
async function writeBrokerNotes(
  ...
  const method = replace ? 'replaceNoteByName' : 'writeNote'
  // Set up webAuthn options
  const webAuthnOptions = {
    keyName: 'webAuthnKey',
    keyType: 'web_authn',
    eacp: {
      web_authn: await user.storage.webAuthn(email, 'Luke'),
    },
  }
  // Set up meta options
  ...
  // Write the notes
  return Promise.all(
    [webAuthnOptions, emailRecoveryOptions, toznyOTPOptions].map(
      async options => {
