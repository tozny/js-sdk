...
class Realm extends CryptoConsumer {
  ...
  async login(username, password, actionHandler, credentialType = 'password') {
    ...
    // denote if the user is a federated user
    // if the server issues a password-challenge then this user
    // is a federated user
    user.config.isFederated = federated;
    return user
  }
   ...
   * @return {Promise<object>} An object containing the proper OIDC redirect URL
   */
  async initiateIdpLogin(redirectURL, scope) {
    const pkceVerifier = await this.crypto.platform.b64URLEncode(
      await this.crypto.randomBytes(32)
    )
    const pkceChallenge = await this.crypto.hash(pkceVerifier, 'SHA-256')
    const request = await fetch(`${this.apiUrl}/v1/identity/idp-login`, {
      method: 'POST',
      body: JSON.stringify({
        realm_name: this.realmDomain,
        app_name: this.appName,
  ...
  async completeIdpLogin(username, password, tokenInfo) {
    const realmInfo = await this.info()
    const { noteName, cryptoKeyPair, signingKeyPair } = await deriveNoteCreds(
      realmInfo.name,
      this.crypto,
    ...
    try {
      let eacpRequest = await anonAuth.tsv1Fetch(
        `${this.apiUrl}/v1/identity/idp-login/${this.realmDomain}/identity/tozideacp`,
        {
          method: 'POST',
          headers: {
            'Keycloak-Token': tokenInfo.access_token,
            'Content-Type': 'application/json',
          },
          body: JSON.stringify(payload),
        }
    ...
    const storedCreds = await credentialNoteCall(
      this.storage.readNoteByName(
        ...
        {},
        {
          'X-TOZID-LOGIN-TOKEN': tokenInfo.access_token,
        },
        this.apiUrl
      )
    )
  ...
  logoutRedirectUrl(params, hostname=null) {
    const allowList = ['post_logout_redirect_uri', 'state', 'initiating_idp', 'id_token_hint', 'id_portal_logout']
    ...
    let logoutBaseUrl = this.apiUrl
    if(hostname != null) {
      logoutBaseUrl = hostname 
    }
    const baseUrl = `${logoutBaseUrl}/auth/realms/${this.realmDomain}/protocol/openid-connect/logout`
    return `${baseUrl}?${urlEncodeData(queryParams)}`
  }

  async eidpCreateToznyIdentity(token) {
    let username = "";
    let restrictedLogin = false;
    ...
    const registerRequest = await anonAuth.tsv1Fetch(
      `${this.apiUrl}/v1/identity/idp-login/${this.realmDomain}/identity/register`,
      {
        method: 'POST',
        headers: {
    ...
    if (registerRequest.status == 200) {
      ...
      const identityConfig = new Config(
        ...
        this.appName,
        this.apiUrl,
        registerResponse.identity.name,
        registerResponse.identity.id,
        this.brokerTargetUrl,
        registerResponse.identity.first_name,
        registerResponse.identity.last_name
      )
      const storageClientConfig = new this.storage.Config(
        registerResponse.identity.tozny_id,
        registerResponse.identity.api_key_id,
        registerResponse.identity.api_secret_key,
        cryptoKeys.publicKey,
        cryptoKeys.privateKey,
        signingKeys.publicKey,
        signingKeys.privateKey,
      ...
      await writeBrokerNotes(
        idClientRegister,
        registerResponse.identity.name,
        registerResponse.realm_broker_identity_tozny_id,
        false,
        60
      )
