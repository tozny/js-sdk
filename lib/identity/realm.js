...
class Realm extends CryptoConsumer {
  ...
  async login(username, password, actionHandler, credentialType = 'password') {
    ...
    const sessionRequest = await fetch(
      `${this.apiUrl}/auth/realms/${this.realmDomain}/protocol/openid-connect/auth`,
      {
        method: 'POST',
   ...
   * @param {object} options The options for to send when issuing the challenge to recover the account
