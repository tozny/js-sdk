...
class Realm extends CryptoConsumer {
   ...
   * @param {int} email_eacp_expiry_minutes The number of minutes email eacps should be valid for
   *
   *
  ...
  async register(
    ...
    const idClient = new this.Client(idConfig, storageClient)
    // Login note
      ...
      brokerInfo.signingKey.ed25519,
      {
        ...
        eacp: {
          email_eacp: {
            ...
            expiry_minutes: email_eacp_expiry_minutes,
          },
        },
      }
    )
   ...
   */
  async login(username, password, redirectUrl, credentialType = 'password') {
    const { signingKeyPair } = await deriveNoteCreds(
      this.realmName,
      this.crypto,
      username,
      password,
  ...
  async completeLogin(username, password, token, credentialType = 'password') {
    const { noteName, cryptoKeyPair, signingKeyPair } = await deriveNoteCreds(
      this.realmName,
      this.crypto,
      username,
  ...
  fromObject(obj) {
    ...
    return new this.Client(idConfig, storageClient)
  }
