...
class Client extends PartialClient {
  ...
  async createSecret(secret) {
    ...
    if (groups.groups.length < 1) {
      ...
      console.log('eek! this is making a group that exists!')
      ...
      // eslint-disable-next-line no-console
      console.log('did it work anyways?!')
    } else {
  ...
  async getSecrets(limit, nextToken) {
    ...
    for (var index = lengthGroup - 1; index >= 0; index--) {
      // eslint-disable-next-line no-console
      console.log('sharedRecors', groupList.groups)
      let sharedRecords = await this.storage.listRecordsSharedWithGroup(
        groupList.groups[index].groupID
      )
      // eslint-disable-next-line no-console
      console.log('sharedRecords', sharedRecords)
      if (sharedRecords[0] !== undefined) {
        for (
          var recordIndex = 0;
          recordIndex < sharedRecords[0].length;
   ...
   */
  async removeSingleSecret(secretID, version, secretName, secretType) {
    // console.log('in function')
    const sharedList = await this.getSecretSharedList(secretName, secretType)
    await Promise.all(
      sharedList.map(async users => {
        let identity = await this.searchIdentityByUsername(users.username)
        const groupName = `tozny.secret.${this.config.realmName}.${this.storage.config.clientId}.${identity.client_id}.${secretName}.${secretType}`
        ...
        if (sharedRecords[0].length < 2) {
          await this.revokeSecretFromUser(
            secretName,
            secretType,
            users.username
          )
    ...
    // if (sharedRecords[0].length < 2) {
    //   // eslint-disable-next-line no-console
    //   console.log('revoking from admin group', secretType, this.storage.config)
    //   await this.revokeSecretFromUser(secretName, secretType, 'alannah1')
    // }
    return await this.storage.deleteRecord(secretID, version)
  }
}
