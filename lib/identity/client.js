...
class Client extends PartialClient {
   ...
   * @return {AccessRequest} The created access request
   */
    async createRoleAccessRequest(realmName, role, reason) {
      const response = await this.storage.authenticator.tsv1Fetch(
        `${this.storage.config.apiUrl}/v1/identity/pam/access_requests/role/create`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
  ...
  async getOpenRoleAccessRequest(nextToken = 0, limit = 1000) {

    const response = await this.storage.authenticator.tsv1Fetch(
        `${this.storage.config.apiUrl}/v1/identity/pam/access_requests/role/open`,
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
