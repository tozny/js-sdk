...
class Client extends PartialClient {
   ...
   * @return {GroupMember[]}
   */
  async shareSecretWithUsername(secretName, secretType, usernamesToAdd = []) {
    ...
    const clientMapping = await this.searchRealmIdentitiesByUsername(
      usernamesToAdd
    )
    ...
    usernamesToAdd.forEach(username => {
      ...
      let member = new GroupMember(clientID, capabilities)
      groupMembers.push(member)
    })
