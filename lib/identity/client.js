...
async function refreshAgentToken(client) {
  ...
  const response = await credentialedDecodeResponse(request)
  // record the time this token and refresh token will expire based on the info
  // response. Since we set this date based relative times, adding 5 seconds
  // helps ensure we do not attempt to use a token that is expired but shows as
  // valid due to network latency.
  const newExpiry = Math.floor(Date.now() / 1000) + response.expires_in - 5
  const newRefreshExpiry = Math.floor(Date.now() / 1000) + response.refresh_expires_in - 5
  return {
