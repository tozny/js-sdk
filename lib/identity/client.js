...
const { GroupMember, Search, errors } = require('../../types')
...
class Client extends PartialClient {
    ...
    for (var index = lengthGroup - 1; index >= 0; index--) {
      ...
      try {
        ...
        let errMessage =
          ...
          ' with error ' +
          e.message
        processingErrs.push(errMessage)
        continue
      }
      if (sharedRecords[0] !== undefined) {
        for (
  ...
  /**
   * validUserForSecretSharedList checks if the username from this group should be added to the list of usernames
   *
   * @param {object} group
   * @param {string} secretName
   * @param {string} secretType
   ...
   * @return {Boolean}
   */
  validUsernameForSecretSharedList(group, secretName, secretType) {
    ...
    for (let index = 0; index < groupList.groups.length; index++) {
      if (
        ...
        if (recordsShared.length > 0) {
          ...
          if (
            this.validUsernameForSecretSharedList(
   ...
   */
  async revokeShareBeforeDeleteSecret(groupName, recordType, secretId) {
  ...
  async deleteSecretVersion(secret) {
    ...
    if (identities.searched_identities_information) {
      await Promise.all(
        identities.searched_identities_information.map(async (identity) => {
          ...
          await this.revokeShareBeforeDeleteSecret(
            groupName,
            secret.meta.type,
    ...
    const groupName = `tozny.secret.${this.config.realmName}.${this.storage.config.clientId}`
    await this.revokeShareBeforeDeleteSecret(
      groupName,
      secret.meta.type,
      secret.meta.recordId
