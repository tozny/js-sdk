...
const {
  ...
  AccessRequestGroupsRequest,
  AccessRequestGroupsResponse,
  Group,
} = require('../../types')
...
class Client extends PartialClient {
  ...
  async approveAccessRequests(realmName, approvals) {
    ...
    return data.access_requests.map((ar) => AccessRequest.decode(ar))
  }
  ...
  async availableAccessRequestGroups(realmName) {
    ...
    await checkStatus(response)
    const data = await response.json()
    return data.groups.map(({ group_id, group_name }) => ({
      id: group_id,
