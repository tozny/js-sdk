...
class Client extends PartialClient {
  ...
  */
  ...
  /**
     * getAccessControlApplicationPolicy fetches the current state of an applications access control policy 
     *
     * @param {Object} applicationID The id of the application/client 
     *
     * @return {Object} status
     */
  async getAccessControlApplicationPolicy(applicationID) {
    let response = await this.storage.authenticator.tsv1Fetch(
      `${this.storage.config.apiUrl}/v1/identity/realm/${this.config.realmName}/application/${applicationID}/access-control`,
      {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
