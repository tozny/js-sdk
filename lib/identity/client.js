...
class Client extends PartialClient {
   ...
   * @param {String} secretType
   * @param {String} secretName
   * @param {String} userToRevokeShare
   *
   * @return {Boolean}
   */
  async revokeRecordWithGroup(secretName, secretType, userToRevokeShare) {
    // Validation
  ...
  async getSecretSharedList(secretName, secretType) {
    ...
    if (groupList.groups.length < 1) {
      // If there is only one group, this means its the automatic group, no shared groups
      return []
    } else {
      let groupSharedList = []
