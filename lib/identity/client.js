...
class Client extends PartialClient {
  ...
  async agentInfo() {
    try {
      ...
      // Preserve the error and propagate it to the application
      throw error instanceof Error ? error : new Error(error)
    }
  }
  ...
  async createCertificate(data) {
    try {
      ...
      let response = await this.storage.authenticator.tsv1Fetch(
        `${this.storage.config.apiUrl}/v1/identity/realm/info/create-certificate`,
        {
          method: 'POST',
          body: JSON.stringify({
            data: data,
      ...
      throw new Error(`Failed to create certificates blob - ${error.message}`)
    }
  }
  ...
  */
  ...
  async deleteIdentity(identityId) {
    const response = await this.storage.authenticator.tsv1Fetch(
      `${this.storage.config.apiUrl}/v1/identity/admin/realm/${this.config.realmName}/identity/${identityId}`,
      {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
  ...
  async sendNotesViaEmail(clientId, payload) {
      const response = await this.storage.authenticator.tsv1Fetch(
        `${this.storage.config.apiUrl}/v1/identity/realm/info/${this.config.realmName}/sendNote/${clientId}`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
