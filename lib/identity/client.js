...
class Client extends PartialClient {
  ...
  async getSecrets(limit) {
    ...
    let searchType = `tozny.secrets.${SECRET_UUID}.`
    request.match({ type: searchType }, 'OR', 'FUZZY')
    const resultQuery = await this.storage.search(request)
    const secretList = await resultQuery.next()
    return secretList, resultQuery.request.nextToken
  }
  async loadMoreSecrets(limit, nextToken) {
    const request = new Search(true, true, limit)
    let searchType = `tozny.secrets.${SECRET_UUID}.`
    request.match({ type: searchType }, 'OR', 'FUZZY')
    const resultQuery = await this.storage.search(request)
    resultQuery.request.nextToken = nextToken
    const secretList = await resultQuery.next()
    return secretList, resultQuery.request.nextToken
  }
}
